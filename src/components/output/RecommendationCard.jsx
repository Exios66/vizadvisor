  ...
  return (
    <div className="rounded-xl border border-indigo-500/30 bg-indigo-500/5 p-5 space-y-4">
      <div className="space-y-2">
        ...
        <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">{chart_type}</h2>
        <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">{rationale}</p>
      </div>

      {mappings.length > 0 && (
        <div>
          <h3 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Data Mapping</h3>
          <div className="grid grid-cols-2 gap-1.5">
            {mappings.map(([channel, value]) => (
              <div key={channel} className="flex items-start gap-2 bg-slate-100 dark:bg-slate-800/60 rounded-lg px-3 py-2">
                <span className="text-xs text-slate-500 capitalize min-w-[48px] pt-0.5">{channel.replace('_', ' ')}</span>
                <span className="text-xs text-slate-800 dark:text-slate-200 font-mono break-words">
